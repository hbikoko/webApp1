...
function WWD() {
  ...
  return (
    <div className="wwd-container">
      <h2 className="wwd-header">{t('wwd.header', 'What We Do')}</h2>
      <div className="services-container">
        {servicesData.map((service) => (
          <Link to={service.path} key={service.title} style={{ textDecoration: 'none', color: 'inherit' }}>
            <ServiceBlock
              icon={service.icon}
              title={t(`wwd.${service.title.replace(/\s+/g, '').toLowerCase()}`, service.title)}
            />
          </Link>
        ))}
      </div>
      <button className="wwd-read-more" aria-label="Read more about our services">
        <a href="/services">{t('wwd.readMore', 'Read More About What We Do')}</a>
      </button>
    </div>
  );
}
